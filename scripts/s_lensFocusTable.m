...
% All the negative values could not be focused.  We set to Nan
...
vcNewGraphWin;
loglog(objDistance,allFilmDistance');
...
for ii=1:length(lensFiles)
    ...
    filmDistance = allFilmDistances(ii,:);   % This lens distance
    
    idx = find(isnan(filmDistance),1,'last'); % First NaN
    closestDistance = saveObjDistance(idx);
    
    idx = find(~isnan(filmDistance),1,'first'); % Remove NaNs
